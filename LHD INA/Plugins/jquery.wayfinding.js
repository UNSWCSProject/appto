...
//			console.log("options loaded: ", action, passed, options);
			...
			if (action && typeof(action) === 'string') {
				...
				case 'currentMap':
					//return and set
					if (passed === undefined) {
						result = $("div:visible", obj).attr('id');
					} else {
		...
		function initialize(target) {
			...
			target.attr('style', 'margin-left:1920px'); // find a better way to allow the objects to be in the dom and visible, without using this trick?
			...
			$.each(maps, function (i, floor) {
				...
				maps[i].svgHandle = targetFloor.svg('get');

				//provide time after callback for loaded SVG to make its way into DOM
				// need to look further into this for timing and refactoring to a callback
				maps[i].svgHandle.load(floor.path, setTimeout(function () {
		...
		function finishFloor(el, mapNum, floor) {
			...
			$("#" + floor.id + " #Rooms a", el).each(function () {
				if ($(this).attr('id') && $(this).attr('id').indexOf('_') > 0) {
					...
					$(this).attr('id', oldID.slice(0, oldID.indexOf('_')));
				}
			});
			...
			$("#" + floor.id + " #Doors line", el).each(function (index, line) {
				...
				$.each(dataStore.paths[mapNum], function (index, path) {
					if (floor.id === path.floor && 
						((path.ax === x1 && path.ay === y1) || 
						(path.ax === x2 && path.ay === y2)))
					{
						path.doorA.push(doorId);
					} else
					if (floor.id === path.floor && 
						((path.bx === x1 && path.by === y1) || 
						(path.bx === x2 && path.by === y2)))
					{
			...
			$("#" + floor.id + " #Portals line", el).each(function (index, line) {
				...
				portal.type = $(this).attr('id').split('.')[0];
				portal.floor = floor.id;
				
				portal.mate = $(this).attr('id').split('.').slice(0, 2).join('.') + '.' + floor.id;
				...
				matches = $.grep(dataStore.paths[mapNum], function (n, i) {
					return ((floor.id === n.floor && x1 === n.ax && y1 === n.ay) || (floor.id === n.floor && x1 === n.bx && y1 === n.by));
				});
		...
		function buildPortals(el) {
				...
				if (outerSegment.matched === false) {
					...
					for (segmentInnerNum = segmentOuterNum; segmentInnerNum < portalSegments.length; segmentInnerNum++)
					{
						if (portalSegments[segmentInnerNum].id === outerSegment.mate && portalSegments[segmentInnerNum].mate === outerSegment.id) {
							...
							portal.accessible = (portal.type === 'Elev') ? true : false;
			...
			$("#Rooms a", el).click(function (event) {
				$(el).wayfinding('routeTo', $(this).attr('id'));
				event.preventDefault();
			});
			
			$(el).attr('style', 'margin-left:0px');
		...
		function routeTo(destination) {
			...
			startPaths,
			endPaths,
			sourceFloor,// = source.parents("div").attr("id");
			destinationFloor,// = destination.parents("div").attr("id");
			sourcemapNum,
			destinationmapNum,
			draw,
			doorANum,
			...
			//clear all rooms
			$("#Rooms *[class=wayfindingRoom]", obj).removeClass('wayfindingRoom'); 
...
//								maps[level[0].floor].svgHandle.circle(stroke.x,stroke.y,2);
						...
						if (i > 0) {
							// handle this as a setTimeout?

							directionSwap = setTimeout(function () {
								$("#" + maps[drawing[i - 1][0].floor].id, obj).hide();
								$("#" + maps[drawing[i][0].floor].id, obj).show();
							}, Math.round(drawing[i].routeLength * options.path.speed));

						}

						thisPath = maps[level[0].floor].svgHandle.path(path, {'stroke': options.path.color, 'strokeWidth': options.path.width, fill: 'none', 'class': 'directionPath'});
						...
						//animate path
						$(drawing[i].path).animate({svgStrokeDashOffset: 0}, drawLength * options.path.speed);
